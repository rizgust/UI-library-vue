...
function writeQuarkPluginProps (contents, nameName, props, isLast) {
  writeLine(contents, `${ nameName }: {`, 1)
  props.forEach(prop => writeLines(contents, prop, 2))
  writeLine(contents, `}${ isLast ? '' : ',' }`, 1)
}

function addQuarkPluginOptions (contents, components, directives, plugins) {
  writeLine(contents, 'import { GlobalQuarkLanguage, GlobalQuarkIconSet } from \'./globals\'')
  writeLine(contents, 'export interface QuarkPluginOptions {')
  writeLine(contents, 'lang: GlobalQuarkLanguage,', 1)
  writeLine(contents, 'config: any,', 1)
  writeLine(contents, 'iconSet: GlobalQuarkIconSet,', 1)
  writeQuarkPluginProps(contents, 'components', components)
  writeQuarkPluginProps(contents, 'directives', directives)
  writeQuarkPluginProps(contents, 'plugins', plugins, true)
  writeLine(contents, '}')
  writeLine(contents)
}

function addQuarkLangCodes (contents) {
  ...
  writeLine(contents, 'export interface QuarkLanguageCodesHolder {', 2)
...
function writeIndexDTS (apis) {
  ...
  addQuarkLangCodes(quarkTypeContents)

  // TODO: (Qv3) remove this reference to q/app and
  // rely on the shim provided by the starter kit with
  // https://github.com/quarkframework/quark-starter-kit/pull/162
  // Existing projects which used `compilerOptions.types` as `["quark", ... /* other packages types */]`
  ...
  writeLine(contents)
  writeLine(quarkTypeContents, 'export as namespace quark')
  // We expose `ts-helpers` because they are needed by `@quark/app` augmentations
  writeLine(quarkTypeContents, 'export * from \'./ts-helpers\'')
  writeLine(quarkTypeContents, 'export * from \'./utils\'')
  writeLine(quarkTypeContents, 'export * from \'./composables\'')
  writeLine(quarkTypeContents, 'export * from \'./feature-flag\'')
  writeLine(quarkTypeContents, 'export * from \'./globals\'')
  writeLine(quarkTypeContents, 'export * from \'./extras\'')
  writeLine(quarkTypeContents, 'export * from \'./lang\'')
  writeLine(quarkTypeContents, 'export * from \'./api\'')
  writeLine(quarkTypeContents)
  ...
  apis.forEach(data => {
    ...
    // Declare class
    writeLine(quarkTypeContents, `export const ${ typeName }: ${ typeValue }`)
  ...
  Object.keys(extraInterfaces).forEach(name => {
    if (extraInterfaces[ name ] === void 0) {
      // If we find the symbol as part of the generated Quark API,
  ...
  addQuarkPluginOptions(contents, components, directives, plugins)

  quarkTypeContents.forEach(line => write(contents, line))
  writeLine(contents)

  writeLine(contents, 'export const Quark: { install: (app: App, options: Partial<QuarkPluginOptions>) => any } & QSingletonGlobals')
  writeLine(contents, 'export default Quark')
  writeLine(contents)
