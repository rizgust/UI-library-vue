...
process.on('exit', code => {
  if (code === 0 && tableData.length > 0) {
    ...
    console.log()
    console.log(` Summary of Quark v${ version }:`)
    console.log(output)
  }
})
...
function getDestinationInfo (dest) {
  ...
  if (dest.endsWith('.js')) {
    return {
      ...
      toTable: dest.indexOf('dist/quark') > -1
    }
  }
...
module.exports.rollupQuarkUMD = function (config = {}) {
  return {
    name: 'quark-umd',
    transform (code) {
      return {
        code: `Quark.${ config.type }.set(${ code.replace('export default ', '') })`
      }
    }
  }
}
