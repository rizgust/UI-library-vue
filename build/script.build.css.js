...
function generateUMD (code, middleName, ext = '') {
  return buildUtils.writeFile(`dist/quark${ middleName }${ ext }.css`, code, true)
    .then(code => nano.process(code, { from: void 0 }))
    .then(code => buildUtils.writeFile(`dist/quark${ middleName }${ ext }.prod.css`, code.css, true))
}
...
function generateBase (source) {
  ...
  const sassDistDest = path.join(__dirname, '../dist/quark.sass')
