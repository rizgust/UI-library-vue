...
const topSections = {
  plugin: [ 'meta', 'injection', 'quarkConfOptions', 'addedIn', 'props', 'methods' ],
  component: [ 'meta', 'quarkConfOptions', 'addedIn', 'props', 'slots', 'events', 'methods' ],
  directive: [ 'meta', 'quarkConfOptions', 'addedIn', 'value', 'arg', 'modifiers' ]
}
...
function parseAPI (file, apiType) {
  ...
  for (const type in api) {
    ...
    if ([ 'value', 'arg', 'quarkConfOptions', 'meta' ].includes(type)) {
    ...
    if ([ 'meta', 'quarkConfOptions' ].includes(type)) {
      parseObject({
        banner: `${ banner } "${ type }"`,
        api,
        itemName: type,
