...
const builds = [
  { // Generic prod entry (client-side only; NOT used by Quark CLI)
    rollup: {
      input: {
        input: resolve('src/index.all.js')
      },
      output: {
        file: resolve('dist/quark.esm.js'),
        format: 'es'
      }
    },
    build: {
      minified: true,
      replace: {
        __QUARK_VERSION__: `'${ version }'`,
        __QUARK_SSR__: false,
        __QUARK_SSR_SERVER__: false,
        __QUARK_SSR_CLIENT__: false,
        __QUARK_SSR_PWA__: false
      }
    }
  },
  { // SSR server prod entry
    rollup: {
      ...
      output: {
        file: resolve('dist/quark.cjs.js'),
        format: 'cjs'
      }
    },
    build: {
  ...
  { // UMD entry
    rollup: {
      ...
      output: {
        file: resolve('dist/quark.umd.js'),
        format: 'umd'
      }
    },
    build: {
...
function genConfig (opts) {
  ...
  if (opts.rollup.output.name !== false) {
    opts.rollup.output.name = opts.rollup.output.name || 'Quark'
  }
  else {
...
function injectVueRequirement (code) {
  ...
  const checkMe = ` if (Vue === void 0) {
    console.error('[ Quark ] Vue is required to run. Please add a script tag for it before loading Quark.')
    return
  }
  `
