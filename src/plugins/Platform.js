...
/**
 * __ QUARK_SSR __            -> runs on SSR on client or server
 * __ QUARK_SSR_SERVER __     -> runs on SSR on server
 * __ QUARK_SSR_CLIENT __     -> runs on SSR on client
 * __ QUARK_SSR_PWA __        -> built with SSR+PWA; may run on SSR on client or on PWA client
 *                              (needs runtime detection)
 */

export const isRuntimeSsrPreHydration = __QUARK_SSR_SERVER__
  ? { value: true }
  : ref(
    __QUARK_SSR_CLIENT__ && (
      __QUARK_SSR_PWA__ ? document.body.getAttribute('data-server-rendered') !== null : true
    )
  )
...
const hasTouch = __QUARK_SSR_SERVER__
  ? false
...
const userAgent = __QUARK_SSR_SERVER__
  ? ''
...
// what you are doing.
export const client = __QUARK_SSR_SERVER__
  ? ssrClient
  : {
      userAgent,
